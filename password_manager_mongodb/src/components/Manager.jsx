...
const Manager = () => {
    ...
    const savePass = async () => {
        ...
        await fetch("http://localhost:3000/", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: passAraay.id }) })


        setpasswordArray([...passwordArray, { ...passAraay, id: uuidv4() }])
        console.log(passwordArray)
        await fetch("http://localhost:3000/", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...passAraay, id: uuidv4() }) })
        setpassAraay({ website: "", username: "", password: "" })
    }

    const getPass = async () => {
        let req = await fetch("http://localhost:3000/") 
        let passwr = await req.json()
        console.log(passwr)
        setpasswordArray(passwr)
    ...
    const editPassword = (id) => {
        setpassAraay({ ...passwordArray.filter(i => i.id === id)[0], id: id })
        setpasswordArray(passwordArray.filter(i => i.id !== id))
